<template>
  <div>
    ...
    <section class="min-h-screen flex flex-col items-center justify-center bg-(--background) text-center px-6 md:px-12 pt-28 pb-20 relative overflow-hidden">
      ...
      <div class="inline-flex items-center gap-2 px-4 py-1.5 rounded-full text-xs text-(--primary) border border-blue-500/30 bg-blue-600/8 mb-7">
      ...
      <div class="flex flex-col sm:flex-row items-center gap-3 w-full sm:w-auto mb-14">
        <NuxtLink
          to="/auth/register"
          class="w-full sm:w-auto flex items-center justify-center gap-2 px-7 py-3.5 text-sm font-medium text-white bg-blue-600 hover:bg-blue-700 rounded-xl transition-colors"
        >
          Start for free
          <ArrowRight :size="16" />
        </NuxtLink>
          ...
          <div class="flex min-h-80">
            ...
            <div class="hidden md:flex flex-col w-44 border-r border-[#2A2A2A] p-3 bg-black/20">
              <div class="flex items-center gap-2 px-2 py-1.5 mb-4">
                ...
                <span class="text-[13px] font-semibold text-white">BizKeeper</span>
              </div>
              <div
                v-for="item in sidebarItems"
                :key="item.label"
                class="flex items-center gap-2 px-2 py-1.5 rounded-md text-xs mb-0.5 transition-colors"
                :class="item.active ? 'bg-blue-600/15 text-blue-400' : 'text-[#888]'"
              >
            ...
            <div class="flex-1 p-4 overflow-hidden">
              ...
              <div class="bg-[#0F0F0F] border border-[#2A2A2A] rounded-lg overflow-hidden">
                ...
                <div
                  ...
                  :key="row.supplier"
                  class="grid grid-cols-3 px-2.5 py-1.5 border-b border-[#2A2A2A]/50 last:border-0 text-[10px] items-center"
                >
                  <span class="font-medium text-white">{{ row.supplier }}</span>
                  <span class="text-[#888] text-[9px] font-mono">{{ row.date }}</span>
                  <span class="font-mono font-semibold text-right text-white">{{ row.amount }}</span>
                </div>
              </div>
            </div>
          </div>
    ...
    <section id="features" class="px-6 md:px-12 py-24 bg-[#1A1A1A]">
      <div class="mb-14">
        ...
        <h2 class="text-3xl sm:text-4xl font-extrabold tracking-tight mb-3 text-white">Everything you need to run your records</h2>
      ...
      <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-px bg-[#2A2A2A] rounded-2xl overflow-hidden border border-[#2A2A2A]">
        <div v-for="feature in features" :key="feature.title" class="bg-[#1A1A1A] hover:bg-[#1f1f1f] p-8 transition-colors">
          ...
          <h3 class="text-sm font-semibold mb-2 text-white">{{ feature.title }}</h3>
